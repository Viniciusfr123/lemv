<template>
  <div class="flex justify-center w-full">
    <h1 class="text-4xl font-black text-gray-800"> Criar Competência
    </h1>
  </div>
  ...
  setup (props) {
    ...
    async function handleSubmit () {
      try {
        ...
        const { data, errors } = await services.skill.createOne(state.skill)
        if (!errors) {
          state.isLoading = false
          toast.success(`Competência ${data.data.id} criado com sucesso!`)
