...
export default {
  props: ['id', 'code', 'description', 'abilities', 'lastMofication'],
  ...
  setup (props) {
    ...
    async function handleSubmit () {
      try {
        ...
        const { data, errors } = await services.skill.deleteOne(props.id)
        if (!errors) {
          state.isLoading = false
          state.reponse = data
          modal.close()
          toast.success(`Item ${props.id} deletado com sucesso`)
          router.push({ name: 'AdministradorCompetencias/oi' })
          return
        }
        state.isLoading = false
      } catch (error) {
